...
class NearByControllerViewController: UIViewController, DatabaseDelegate {
    
    @IBOutlet weak var nearbyTableView: UITableView!
    
    let selectedPlaceCategory: [PlaceCategory] = [.store, .repair, .food]
    var placeModel = PlaceModel.instance
    var isLoading = true
...
extension NearByControllerViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0{
            ...
            if !isLoading {
            cell.didFetchRecords(places: placeModel.places.filter({( place : Place) -> Bool in
                return (place.category == selectedPlaceCategory[currentIndex])
                }))
            }
            if !isLoadingImage {
                cell.didFetchImage(places: placeModel.places.filter({( place : Place) -> Bool in
                    return (place.category == selectedPlaceCategory[currentIndex])
                }))
            }
            return cell
